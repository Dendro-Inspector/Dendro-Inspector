...
class ToneProfile(StrEnum):
    ...
    Public is the default so contributors, demos and corporate deployments do not inherit
    a register nobody chose. Selecting the unfiltered profile changes only the vocabulary
    and the register note sent to the model — never a taxon, level or confidence.
    """

    EVIL_DUCK_PUBLIC = "evil_duck_public"
    EVIL_DUCK = "evil_duck"
...
def load_config(overrides: AppConfig | None = None) -> AppConfig:
    ...
    scenario = _env("EVIL_DUCK_FAKE_SCENARIO", "primary-pass")
    primary = ProviderConfig(
        adapter=Adapter(
            _env("EVIL_DUCK_PRIMARY_PROVIDER", Adapter.FAKE.value) or Adapter.FAKE.value
        ),
        model=_env("EVIL_DUCK_PRIMARY_MODEL"),
        api_key_env="OPENAI_API_KEY",
        scenario=scenario,
    )
    arbiter = ProviderConfig(
        adapter=Adapter(
            _env("EVIL_DUCK_ARBITER_PROVIDER", Adapter.FAKE.value) or Adapter.FAKE.value
        ),
        model=_env("EVIL_DUCK_ARBITER_MODEL"),
        api_key_env="ANTHROPIC_API_KEY",
        scenario=scenario,
    )
    trace_dir = _env("EVIL_DUCK_TRACE_DIR")
    ...
    return AppConfig(
        ...
        prompts=PromptConfig(
            ...
            tone_profile=ToneProfile(
                ...
                or ToneProfile.EVIL_DUCK_PUBLIC.value
            ),
        ),
        knowledge=KnowledgeConfig(
            root=Path(_env("EVIL_DUCK_KNOWLEDGE_ROOT", str(DEFAULT_KNOWLEDGE_ROOT)) or "knowledge"),
            region_pack=_env("EVIL_DUCK_REGION_PACK", "eastern-europe"),
        ),
        observability=ObservabilityConfig(
            log_level=_env("EVIL_DUCK_LOG_LEVEL", "INFO") or "INFO",
